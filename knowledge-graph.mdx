...
On top of the knowledge graph, Ravell builds a **product graph** that identifies product-level concepts — problems, features, decisions, and product areas — and tracks the relationships between them.

The product graph uses the same entity resolution and document linking described above, but focuses specifically on product intelligence: ranking problems by priority, detecting blind spots, and measuring the impact of shipped features.

See [Product intelligence](/product-intelligence) for the full details on how Ravell scores problems, detects blind spots, and delivers a weekly brief.
...
<CardGroup cols={2}>
  ...
  <Card title="Product intelligence" icon="chart-mixed" href="/product-intelligence">
    Ranked problems, blind spots, and a weekly brief from the product graph.
  </Card>
</CardGroup>
