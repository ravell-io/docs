...
The Product Intelligence layer sits on top of the Knowledge Graph. It continuously extracts **product entities** — problems, feature requests, and topics — from indexed documents, then scores and ranks them based on evidence volume, confidence, recency, source diversity, and engagement signals.

This powers:

- **Scored problems** — a prioritized view of your product's top issues
- **Blind spot detection** — rising problems with no planned response
- **Weekly briefs** — automated digests delivered to Slack with trends, ranking changes, and merge suggestions

Product intelligence data is also available to the Synthesis Engine and the Slack Agent, so when you ask questions like "What are our top problems?", the answer draws on pre-computed scores and trends rather than searching from scratch.

Learn more in the [Product intelligence guide](/product-intelligence).
...
| **Weekly Briefs** | Slack channel | Product intelligence schedule |

All outputs are evidence-backed: every claim ties to a source. You can click citations to verify, follow up in the same thread, or push reports to Notion when connected. Weekly briefs include interactive elements — reply in the thread to ask follow-up questions about any problem or trend.
...
<CardGroup cols={2}>
  ...
  <Card title="Product intelligence" icon="chart-mixed" href="/product-intelligence">
    Scored problems, blind spots, and weekly briefs.
  </Card>
