...
Data is stored as indexed documents with embeddings for semantic search. When a document is indexed, Ravell extracts entities (people, companies, customers, teams) from metadata and content, resolves them against a unified entity graph, and builds links to related documents. It also identifies product concepts — problems, feature requests, and topics — to power the [product graph](/knowledge-graph#product-graph). Retrieval uses these links to discover evidence you didn't directly search for.
...
The Knowledge Graph stores your indexed documents, entity profiles, and the links between them. It supports:
...
- **Entity resolution** — unify people, companies, and teams across tools using normalization, alias matching, and semantic similarity
...
Built on the knowledge graph, the **product graph** extracts product-relevant concepts — problems, feature requests, and topics — from your indexed documents. It tracks how these evolve over time with priority scoring, velocity trends, and blind spot detection. See [Knowledge graph](/knowledge-graph#product-graph) for details.
...
- **Better Linking** — More document links, entity resolution, semantic deduplication, and cross-source connections
