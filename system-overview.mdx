...
Data is stored as indexed documents with embeddings for semantic search. When a document is indexed, Ravell extracts entities (people, projects, issues, problems, feature requests) from metadata and content, then builds links to related documents. Retrieval uses these links to discover evidence you didn't directly search for.
...
The Product Graph sits on top of the Knowledge Graph and identifies recurring **problems**, **feature requests**, and **topics** from your indexed documents. Each problem is classified by subtype (bug, feature gap, UX friction, or integration issue) and scored by priority based on source diversity, evidence volume, recency, and engagement signals. This layer powers product intelligence queries like "What are the top customer problems?" and proactive blind spot detection — surfacing rising issues that have no planned response. See the [knowledge graph](/knowledge-graph#product-graph) page for details.
