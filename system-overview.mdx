...
```mermaid
flowchart TB
    subgraph Flywheel["Learning Loop"]
        direction TB
        A[More Users]
        B[Better Linking]
        C[More Value]
...
Data is stored as indexed documents with embeddings for semantic search. When a document is indexed, Ravell extracts entities (people, projects, issues) from metadata and content, then builds links to related documents. Ravell also extracts higher-level product concepts — problems, feature requests, and topics — into a **Product Graph** that powers product intelligence briefs and emerging issue detection. Retrieval uses these links to discover evidence you didn't directly search for.
...
The Product Graph sits on top of the Knowledge Graph and extracts product-level concepts from your data:

| Entity type | What it captures |
|-------------|-----------------|
| **Problem** | Customer pain points, bugs, and recurring complaints |
| **Feature request** | Asks and requests extracted from feedback and conversations |
| **Topic** | Grouping categories that connect related problems and requests |

The Product Graph tracks relationships between these entities and scores problems by evidence volume, source diversity, recency, and engagement signals. Problems that lack a corresponding feature request are flagged as **emerging issues**. This powers the product intelligence briefs that agents deliver to your Inbox and Slack.
...
- **Engagement signals** — your interactions with briefs (clicking **Tell me more**, **Mark reviewed**, or dismissing entities) feed back into problem priority scoring

The loop reinforces itself: better answers attract more usage, which improves linking and context, which delivers more value. Engagement signals from product intelligence briefs add a direct feedback mechanism — problems you engage with rise in priority, making future briefs more relevant to what you care about.
